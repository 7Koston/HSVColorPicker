...
public class ColorPreference extends Preference {
  ...
  private void setPreviewColor(View itemView) {
    ...
    LinearLayout widgetFrameView = itemView.findViewById(android.R.id.widget_frame);
  ...
  private Bitmap createPreviewBitmap() {
    ...
    for (int i = 0; i < w; i++) {
      for (int j = i; j < h; j++) {
        c =
            i <= frameWidth || i >= w - frameWidth - 1 || j >= h - frameWidth - 1
                ? Color.GRAY
                : mColor;
        bm.setPixel(i, j, c);
        if (i != j) {
  ...
  protected void onSetInitialValue(@Nullable Object defaultValue) {
    if (defaultValue != null) setColor((Integer) defaultValue);
    else setColor(mColor);
  }
